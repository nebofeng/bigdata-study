...
/**
 * 
 * 针对上一步的结果统计节目每天每分钟的收视指标
 * 
 */
public class AnalyzeCountProgramRating extends Configured implements Tool {
	public static class AnalyzeCountProgramRatingMapper extends
		...
		protected void map(Object key, Text value, Context context)
			...
			// column0 + "@" + date1 + "@" + min2+avgnum3 +reachnum4
	...
	public int run(String[] args) throws Exception {
		...
		if (otherArgs.length < 2) {
			System.err
					.println("Usage: AnalyzeCountProgramRating cache in [<in>...] <out>");
			System.exit(2);
		}
		...
		job.setJarByClass(AnalyzeCountProgramRating.class);
		job.setMapperClass(AnalyzeCountProgramRatingMapper.class);
	...
	public static void main(String[] args) throws Exception {
		int ec = ToolRunner.run(new Configuration(),
				new AnalyzeCountProgramRating(), args);
		System.exit(ec);
	}
}
